...
N_REPS = 100
BASE_PARAM_FILE = "../parameters/10_28_batch_params.json"
...
TIMESTAMP = "param_sweep_10_28_100each"
SWEEP_OUTPUT_DIR = f"../../output/param_sweep_brief_{TIMESTAMP}"
...
# 1. Generate all parameter files (same as before)
rule generate_all_params:
    ...
    run:
        ...
        for combo in PARAM_COMBINATIONS:
            ...
            params["simulation"]["seed"] = base_seed + combo['seed_offset']
...
rule run_simulation_batch:
    ...
    resources:
        ...
        runtime=get_runtime_scaling(600),  # More time for multiple sims
        cpus_per_task=2,
...
# 3. Batched lineage tests
def get_lineage_batch_inputs(wildcards):
    """Get simulation outputs for this lineage batch"""
    batch_id = int(wildcards.batch_id)
    start_idx = (batch_id - 1) * ANALYSIS_BATCH_SIZE  # Use same batch size as analysis
...
rule run_lineage_batch:
    ...
    conda: "phylo"
    resources:
        mem_mb=6000,  # Bit more memory for multiple lineage tests
        runtime=60,
        cpus_per_task=1,
...
# 4. Batched conditional analysis
rule run_analysis_batch:
    ...
    resources:
        ...
        runtime=get_runtime_scaling(120),  # More time for new bridging analysis
        cpus_per_task=1,
    ...
    shell:
        ...
        tail -n +$((START_IDX + 1)) {SWEEP_OUTPUT_DIR}/batch_mapping.txt | head -n $BATCH_SIZE | while IFS=, read -r combo_id p_val rep; do
            ...
            if [ "$SINGLE_LINEAGE" = "True" ]; then
                ...
                # NEW: Updated bridging analysis with 10_28 script and new outputs
                python ../scripts/10_28_calculate_bridging.py \
                    "$OUTPUT_DIR" \
...
# 5. Updated aggregation rules for new bridging output structure
rule aggregate_bridging:
    ...
    resources:
        ...
        runtime=120,
        cpus_per_task=1,
        slurm_partition=get_compute_partition  # Need good memory for aggregation
    run:
        import pandas as pd
        ...
        for combo in PARAM_COMBINATIONS:
            ...
            # NEW: Check main values file (updated filename)
...
rule aggregate_prevalence_timeseries:
    ...
    run:
        ...
        for combo in PARAM_COMBINATIONS:
            ...
            # Check if analysis was successful (NEW: updated filename)
